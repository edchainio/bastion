...
# FIXME All of the following installs need to be run on one line, after the repositories are added to the package manager.
...
sed -i '/\[ v3_ca \]/a subjectAltName = IP: <elk_server_private_ip_address>' /etc/ssl/openssl.cnf
...
cd crossdock # localhost

scp root@174.138.44.177:/etc/pki/tls/certs/logstash-forwarder.crt . # localhost

scp logstash-forwarder.crt root@67.205.151.241:/tmp # localhost

ssh root@67.205.151.241 # localhost

mkdir -p /etc/pki/tls/certs # testmon-02

cp /tmp/logstash-forwarder.crt /etc/pki/tls/certs/ # testmon-02

#   Set-up Filebeat   # # # # # # # # # # # # # # # # # # # # # # # # # # # ##

echo "deb https://packages.elastic.co/beats/apt stable main" | tee -a /etc/apt/sources.list.d/beats.list # testmon-02

wget -qO - https://packages.elastic.co/GPG-KEY-elasticsearch | apt-key add - # testmon-02

apt-get -y update # testmon-02

apt-get -y install filebeat # testmon-02

sed -i '/^\s*paths:/a - \/var\/log\/syslog' /etc/filebeat/filebeat.yml # testmon-02

sed -i '/^\s*paths:/a - \/var\/log\/auth\.log' /etc/filebeat/filebeat.yml # testmon-02

sed -i -e 's/^\s*- \/var\/log\/\*\.log/        #- \/var\/log\/\*\.log/' /etc/filebeat/filebeat.yml # testmon-02

sed -i 's/- \/var\/log\/syslog/        - \/var\/log\/syslog/' /etc/filebeat/filebeat.yml # testmon-02

sed -i 's/- \/var\/log\/auth\.log/        - \/var\/log\/auth\.log/' /etc/filebeat/filebeat.yml # testmon-02

sed -i -e '/^\s*#document_type/s/^.*$/      document_type: syslog/' /etc/filebeat/filebeat.yml # testmon-02

sed -i -e '/^\s*elasticsearch:/s/^.*$/  #elasticsearch:/' /etc/filebeat/filebeat.yml # testmon-02

sed -i -e '/^\s*hosts: \[\"localhost:9200\"\]/s/^.*$/    #hosts: \[\"localhost:9200\"\]/' /etc/filebeat/filebeat.yml # testmon-02

sed -i -e '/^\s*#logstash:/s/^.*$/  logstash:/' /etc/filebeat/filebeat.yml # testmon-02

sed -i -e '/^\s*#hosts: \[\"localhost:5044\"\]/s/^.*$/    hosts: \[\"<elk_server_private_ip_address>:5044\"\]/' /etc/filebeat/filebeat.yml # testmon-02

sed -i -e '/^\s*#bulk_max_size: 2048/s/^.*$/    bulk_max_size: 1024/' /etc/filebeat/filebeat.yml # testmon-02

# Note: workaround (multiple instances of target since I didn't delete the Elastic Search section of the config. file)
sed -i '/^\s*# Optional TLS. By default is off./a tls:' /etc/filebeat/filebeat.yml # testmon-02

sed -i 's/^tls:/    tls:/' /etc/filebeat/filebeat.yml # testmon-02

sed -i -e '/^\s*#certificate_authorities:/s/^.*$/      certificate_authorities: \[\"\/etc\/pki\/tls\/certs\/logstash-forwarder\.crt\"\]/' /etc/filebeat/filebeat.yml # testmon-02

# FIXME this deletes the Elastic Search section from the file by line number, which is precarious
sed -i -e '184,278d' /etc/filebeat/filebeat.yml # testmon-02

service filebeat restart # testmon-02

update-rc.d filebeat defaults 95 10 # testmon-02
